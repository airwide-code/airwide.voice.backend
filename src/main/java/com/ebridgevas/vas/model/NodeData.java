...
import com.ebridgevas.vas.dto.SessionDTO;
...
//    @Override
    ...
    private SessionDTO parse(String json) {
        String edited = json.replaceAll("\\{", "");
        edited = edited.replaceAll("\\}", "");
        edited = edited.replaceAll("[^\\[\\],]+", "\"$0\"");
        edited = edited.replaceAll(":", "\":\"");
        edited = "{" + edited + "}";
        return new Gson().fromJson(edited, SessionDTO.class);
    }
}
